      ...
      <div className="mt-4 space-y-3">
        ...
        {activeTab === "radar" && (
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-4 shadow-lg shadow-black/20 backdrop-blur-md">
            ...
            <div className="mt-3 max-h-[36rem] overflow-x-hidden overflow-y-auto rounded-lg p-1">
                ...
                {Object.values(regions)
                  ...
                  .map((region) => {
                    ...
                    return (
                      <div
                        key={region.id}
                        className="flex min-w-0 flex-col rounded-xl border border-slate-700/50 bg-slate-900/60 p-3 shadow-lg shadow-black/20 backdrop-blur-md"
                      >
                        {/* ROW 1: header — name + flashing status icon */}
                        <div className="flex items-center justify-between gap-2">
                        ...
                        {/* ROW 2: metrics, icons instead of long labels */}
                        <div className="mt-2 flex items-center justify-between rounded-lg bg-slate-800/40 px-2 py-1.5 text-xs tabular-nums text-slate-300">
                          <span title="التنمية">
                            📈 {Math.round(region.developmentIndex)}
                          </span>
                          <span title="الأمن">
                            🛡️ {Math.round(region.securityLevel)}
                          </span>
                          <span title="الاقتصاد الموازي">
                            💰 {Math.round(region.shadowEconomyLevel)}
                          </span>
                        </div>

                        {/* ROW 3: active-event + deadline alerts (conditional) */}
                        {hasActiveEvent && (
                          <div className="mt-2 flex flex-col gap-1.5">
                            {region.crackdownActive && (
                              <span className="w-full rounded-md border border-amber-500/40 bg-amber-500/10 px-2 py-1 text-[11px] font-semibold text-amber-300">
                            ...
                            {securityDeadlineExceeded && (
                              <span className="w-full animate-pulse rounded-md border border-red-500 bg-red-600/20 px-2 py-1 text-[11px] font-bold text-red-300">
                                ⚠️ تجاوز الأجل (الشهر{" "}
                                {region.securityCampaignMonths}): استنزاف حاد،
                                التدخل مطلوب فوراً!
                              </span>
                            )}
                        ...
                        <div className="mt-3 flex flex-wrap gap-1.5">
                          <button
                            ...
                            className={`min-w-[6rem] flex-1 rounded-md px-2 py-1.5 text-[11px] font-bold transition-all ${
                              region.crackdownActive
                                ? "border border-amber-500/50 bg-amber-500/10 text-amber-200 hover:bg-amber-500/20"
                          ...
                          {region.isStriking && (
                            <button
                              ...
                              className="min-w-[6rem] flex-1 rounded-md bg-gradient-to-r from-red-700 to-red-900 px-2 py-1.5 text-[11px] font-bold text-white shadow shadow-red-950/40 ring-1 ring-red-500/40 transition-all hover:from-red-600 hover:to-red-800 disabled:cursor-not-allowed disabled:from-slate-700 disabled:to-slate-700 disabled:text-slate-500 disabled:ring-0"
                            >
                              حل الإضراب
                            </button>
                          )}
                          <button
                            ...
                            className="min-w-[6rem] flex-1 rounded-md border border-sky-500/50 bg-sky-500/10 px-2 py-1.5 text-[11px] font-bold text-sky-200 transition-colors hover:bg-sky-500/20"
                          >
                            فتح الولاية
                          </button>
                        </div>
